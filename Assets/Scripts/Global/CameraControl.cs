...
//        _newPosition = startLocation.position;
        Move(startLocation.position.x - transform.position.x, startLocation.position.y - transform.position.y, .3f);
    }
    ...
    private IEnumerator EndGameCoroutine()
    {
        ...
        Move(endLocation.position.x - transform.position.x, endLocation.position.y - transform.position.y, .3f);
        yield return new WaitForSeconds(3.5f);
        StartCoroutine(FadeTo(coverSprite, 0f, 1f, 3f));

    }
    
    private IEnumerator FadeTo(SpriteRenderer spriteToFade, float startValue, float aValue, float aTime, float delay = 0f)
    {
        float alpha = startValue;
        var firstColor = new Color(1, 1, 1, alpha);
        spriteToFade.color = firstColor;
	...
	void Update () 
    {
        ...
        if (IsMoving)
        {
            ...
            MainAxis.position = Vector2.MoveTowards(MainAxis.position, _newPosition, Time.deltaTime * _currentMoveSpeed);
        ...
        if (_isShaking)
        {
            ...
            ShakeAxis.localPosition = Vector2.MoveTowards(ShakeAxis.localPosition, _nextShakePosition, Time.deltaTime * _shakeSpeed);
